...
{
    public class ClientSimPlayerManager
    {
        ...
        public static void InitializePlayer(VRCPlayerApi player)
        {
            ...
            if (masterID == -1)
            {
                ...
                masterID = id;
                Debug.Assert(player.isMaster, "ClientSimPlayerManager:InitializePlayer Player should be considered master!");
            }
            else
            {
                Debug.Assert(!player.isMaster, "ClientSimPlayerManager:InitializePlayer Player should not be considered master!");
            }

            Debug.Assert(player.playerId == id, "ClientSimPlayerManager:InitializePlayer Player's id does not match assigned id!");

            if (networkReady)
            {
                ClientSimMain.PlayerJoined(player);
            }
            else
            {
        ...
        public static VRCPlayerApi CreateNewPlayer(bool local, GameObject playerObj, string name = null)
        {
            ...
            Debug.Assert(player.isLocal == local, "ClientSimPlayerManager:CreateNewPlayer New player does not match local settings!");
        ...
        public static void RemovePlayer(VRCPlayerApi player)
        {
            if (masterID == player.playerId)
            {
                ...
                {
                    masterID = VRCPlayerApi.AllPlayers[1].playerId;
                }
            }

            ClientSimMain.PlayerLeft(player);
        ...
        {
            return ClientSimSettings.Instance.isInstanceOwner;
        }
        ...
        public static void EnablePickups(VRCPlayerApi player, bool enabled)
        {
            ...
            player.GetPlayerController().EnablePickups(enabled);
        }
        ...
        public static void UseAttachedStation(VRCPlayerApi player)
        {
            ...
            GameObject obj = VRC.Udon.UdonManager.Instance.currentlyExecuting.gameObject;
            if (obj != null)
            {
                ClientSimStationHelper stationHelper = obj.GetComponent<ClientSimStationHelper>();
                if (stationHelper != null)
                {
                    stationHelper.UseStation(player);    
                }
            }
        }
        ...
        public static List<int> GetPlayersWithTag(string tagName, string tagValue)
        {
            ...
            {
                if (player.GetClientSimPlayer().HasTag(tagName, tagValue))
                {
